...
public class DemoProgram {
	...
	public static void main(String[] args) throws ParseException {
		...
		GasStation omv = new GasStation("OMV");//suzdavane na benzinostanciata
		
//		omv.printVignette();
		
		Driver[] drivers = new Driver[20];//suzdavane na 20 shoiora
		
		createDrivers(drivers, omv);//zadavane stoinosti na svoistvata im
		
		Venchile[] venchiles = new Venchile[200];//suzdavane na 200 prevozni sredstva
		
		createVenchiles(venchiles);//zadavane stoinosti na svoistvata im
		
		for(int i = 0; i < drivers.length; i++){
			System.out.println(drivers[i].getName() + " "+ drivers[i].getMoney());//printirane na vsichki shofiori i tehnite pari
		}
		
//		displayInfo(venchiles);
		
		//zadavane na po 10 prevozni sredstva na shofior
		...
		for(int i = 0; i < drivers.length; i++){
			drivers[i].displayVenchiles();//printirane na prevoznite sredstva na vseki shofior
			System.out.println();
		}
...
//		System.out.println("The starting turnover for today is " + omv.getTurnoverForToday() + ".");
		...
		purchaseVignettes(drivers);//kupuvane na vinetki
		
		System.out.println();
		
		for(int i = 0; i < drivers.length; i++){
			drivers[i].displayVenchiles();//printirane na vsichki prevozni sredstva s vinetki za vseki shofior
			System.out.println();
		}
		
		System.out.println("The final turnonver for today is " + omv.getTurnoverForToday() + ".");//printirane na oborota na benzinastanciata
		
		System.out.println();
		
		omv.printVignette();//printirane na vsichki ostanali vinetki v benzinostanciata
		
		System.out.println();
		
		for(int i = 0; i < drivers.length; i++){
			drivers[i].printTrucksWithExpiredVignettes();
			System.out.println();
		}
